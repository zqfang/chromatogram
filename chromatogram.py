...
class Chromaotogram(object):
    ...
    def __init__(self, abi, seq_range=(0, 50),
                 rev_complement=False, figsize=(10, 5), **kwargs):
    ...
    def _reverse_complement(self):
        ...
        """
        ## {G: DATA9, A: DATA10, T:DATA11, C: DATA12}
        self.sequence = self.record.reverse_complement().seq
        # CTAG
        self.channel_colors = ['blue', 'red', 'green', 'black']
        for ch in self.channels:
            self.trace[ch] = list(reversed(self.record.annotations['abif_raw'][ch]))
        # reversed the peak location index of reversed complement sequence
        ch_len = len(self.trace[self.channels[0]])
        self.base_peak_index = list(reversed([ch_len - j - 1 for j in self.base_peak_index]))

    def plot(self, seq_range=None, rev_complement=False,
        ...
        for i, peak in zip(range(self.start, self.end),
                           self.base_peak_index[self.start:self.end]):
...
if __name__ == "__main__":
    ...
    args = sys.argv
    abi = Chromaotogram(args[1], (args[3], args[4]))
    abi.plot(filename=args[2])
